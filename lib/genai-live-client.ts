...
import { useUser, useAgent } from './state';
...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  public async connect(config: LiveConnectConfig): Promise<boolean> {
    ...
    try {
      ...
      this.emit('error', e instanceof Error ? (e as any) : new ErrorEvent('error', { message: String(e) }));
      return false;
    }
  ...
  protected log(type: string, message: string | object) {
    ...
    // Note: We access state directly here to get current context without passing it down through every call
    const userState = useUser.getState();
    const agentState = useAgent.getState();
    
    // Don't log high-frequency audio buffer events to DB to save quota/noise
    if (type !== 'server.audio' && type !== 'client.realtimeInput') {
      try {
        addDoc(collection(db, 'logs'), {
          timestamp: new Date(),
          type,
          message: typeof message === 'object' ? JSON.stringify(message) : message,
          user: userState.name || 'Anonymous',
          agentId: agentState.current.id,
          agentName: agentState.current.name
        }).catch(e => console.warn("Failed to log to Firestore", e));
      } catch (err) {
        // Silent fail for logging to avoid interrupting app flow
      }
    }
  }
}